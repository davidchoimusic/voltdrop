...
const CEC_CONDUIT_RESULT_TEXT = {
  tooLarge: 'more than the largest listed {conduit} can legally hold',
  tooLargeMixed: '<p>The combined conductor area is {area} mm², which exceeds the {percent}% fill limit of even {largestSize} {conduit}.</p>',
  smallestMixed: 'smallest {conduit} for {count} conductors',
  fillAtSize: '{percent}% of {size}',
  oneWireLimit: '{percent}% for 1 conductor',
  twoWireLimit: '{percent}% for 2 conductors',
  manyWireLimit: '{percent}% for 3+ conductors',
  ...
  totalBreakdown: '<span><strong>TOTAL CONDUCTOR AREA</strong></span><span><strong>{area} mm²</strong></span>',
  mathMixed: '\n<p>Each row uses the conductor cross-section from CSA C22.1:24 {conductorTable}.</p>\n<div class="formula">{breakdown}\nTOTAL = {needed} mm²\n{conduitSize} {conduit} inside area = {conduitArea} mm² (derived from its internal diameter in CSA C22.1:24 Tables 9A–9H)\nAllowed by Rule 12-910 and Table 8 for {count} conductors: {conduitArea} × {percent}% = {allowed} mm²\n{needed} ≤ {allowed}  →  fits at {actualPercent}% fill</div>',
};

const CEC_UI_TEXT = {
  conductorTypeLabel: 'Conductor type',
  conductorTypeHint: 'RW90 is the common Canadian building wire. Choose T90 for nylon-jacketed wire.',
  rigidPvc: 'Rigid PVC',
...
function calc() {
  ...
  $('result-grid').innerHTML = [
    ['Fill at this size', vdFormat(resultText.fillAtSize, { percent: pct.toFixed(1), size: pick.size })],
    ['Legal limit', vdFormat(limitPattern, { percent: Math.round(limit * 100) })],
    ['Wire area total', vdFormat(
      country === 'ca' ? resultText.squareMillimetres : resultText.squareInches,
      { area: needed.toFixed(gridDecimals) },
    )],
    nextUp
      ? ['Easier pull: next size up', nextUp[0]]
      : ['This is the largest size', family.sizes.at(-1)[0]],
