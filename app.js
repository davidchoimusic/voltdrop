...
const DROP_TEXT = {
  ...
  continuousRuleNote: '<strong>NEC continuous load: 125% of the load.</strong> Before: {load} A entered load. After: {load} A × {factor}% = {after} A required ampacity. The switch treats the entire entered load as continuous; it does not model a mixed load.',
  continuousRuleMath: '<h3>Continuous-load step</h3>\n<div class="formula">Entered load: {load} A\n× {factor}%\n= {after} A required ampacity\n{after} A ≤ {permitted} A permitted ampacity → passes</div>\n<p>The switch treats the entire entered load as continuous; it does not model a mixed load.</p>',
