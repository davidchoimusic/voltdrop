...
function applyBfCountry() {
  ...
  Object.keys(src).forEach((label) => {
    ...
    opt.value = label;
    opt.textContent = ca ? `${label} (${src[label]} mL each)` : `${label} (${src[label]} cu in each)`;
...
function calc() {
  ...
  const u = ca ? 'mL' : 'cu in';
  $('big-number').textContent = fmt(needed, ca ? 1 : 2) + ' ' + u;
  $('big-label').textContent = 'needed vs ' + fmt(boxVol, 1) + ' ' + u + ' available';

  const rows = ca
    ? [
        ['Insulated wires', conductors + ' × ' + unit + ' = ' + fmt(conductors * unit, 1)],
        ['Devices (count double)', devices + ' × 2 × ' + unit + ' = ' + fmt(devices * 2 * unit, 1)],
        ['Marrette pairs', Math.floor(marrettes / 2) + ' × ' + unit + ' = ' + fmt(Math.floor(marrettes / 2) * unit, 1)],
        ['Box usage', fmt(pct, 0) + '%'],
      ]
    : [
  ...
  $('math-body').innerHTML = ca ? `
<p>Each ${size} insulated conductor requires <strong>${unit} mL</strong> of box space (CEC Table 22, Rule 12-3034).</p>
<div class="formula">${conductors} wires + ${devices} device${devices === 1 ? '' : 's'} × 2 + ${Math.floor(marrettes / 2)} marrette pair${Math.floor(marrettes / 2) === 1 ? '' : 's'}
= ${counts} total counts × ${unit} mL
= ${fmt(needed, 1)} mL needed
${boxName}: ${fmt(boxVol, 1)} mL available → ${ok ? 'FITS' : 'DOES NOT FIT'}</div>
