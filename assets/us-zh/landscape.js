...
const LS_TEXT = {
  volts: '{volts} V',
  voltsAt: '{label}处 {volts} V',
  feet: '{feet} ft',
  amps: '{amps} A',
  percent: '{percent}%',
  lowestLabel: '最低电压，位于{label}',
  ...
  colCable: '电缆',
  colLowestVolts: '最低 V',
  colSpreadVolts: '电压差 V',
  colCableFeet: '电缆 ft',
  colTotalLoad: '总负载',
  colWorstDrop: '最大电压降',
  hubAtFt: '集线（位于 {feet} ft）',
  hubSuggested: '集线（建议位于 {feet} ft）',
  ...
  cautionWatch: '这里的电压降已经大到值得在埋设电缆前再核对一遍。',
  cautionUnreliable: '在这么大的电压降下，固定电流算法不再可靠：LED 驱动器在电压下降时会吸取更多电流，因此实际电压降比所显示的更严重。请加大规格、缩短回路，或分担负载。',
  cautionCollapse: '此设计无法工作。电缆根本无法在该距离上输送这么大的负载 — 电压会崩塌。请更改导线规格、距离或灯具数量。',
  cautionAmpacity: '本工具只回答电压降。它不检查电缆是否具备承载该电流的额定值，也不检查变压器的额定容量。',
  cautionLimitsUnset: '未按制造商限值进行评估。请输入灯具的最低和最高电压，以便检查每个分接点。',
  cautionBelowMin: '您有 {count} 个分接点低于所输入的 {volts} V 最低值。',
  cautionAboveMax: '您有 {count} 个分接点高于所输入的 {volts} V 最高值。过电压会缩短灯具寿命，卤素灯尤其明显。',
  needTwoRows: '请至少为一个灯具输入距离和负载。',
  badInput: '这些数字无法构成可计算的回路。请检查距离和负载。',
  mathIntro: '每一段电缆都承载其之外所有灯具的电流，因此电压降是逐段计算，再沿路径累加到每个灯具处。',
  mathFormula: '每段电压降 = 2 × K × A × ft ÷ cmil',
  mathConstants: '{material}的 K = {k}。电缆 = {size}（{cm} cmil）。距离为单程；系数 2 对应往返的一对导线。',
  mathNaive: '作为对比，把全部 {amps} A 都放在 {feet} ft 的全长处 — 也就是普通计算器的做法 — 会得出 {volts} V 的电压降，这偏大了。',
  materialCopper: '铜',
  materialAluminum: '铝',
  badgeOk: '正常',
  badgeCheck: '需检查',
