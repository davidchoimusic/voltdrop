...
const SYSTEMS = {
  dc:  { name: 'DC', mult: 1, usesPF: false, multLabel: '', hint: 'DC: baterías, solar, vehículos, LED tiras.' },
  ac1: { name: 'AC monofásica', mult: 1, usesPF: true, multLabel: '', hint: 'monofásica: normal household y light commercial circuitos.' },
  ac3: { name: 'AC trifásica', mult: SQRT3, usesPF: true, multLabel: '√3 × ', hint: 'trifásica: commercial y industrial. ingrese entre líneas tensión.' },
};
const POWER_RESULT_TEXT = {
  ...
  ampsFormula: 'Amperios = vatios ÷ ({mult}voltios)\n     = {watts} ÷ ({mult}{volts})\n     = {amps} A',
  ampsPfFormula: 'Amperios = vatios ÷ ({mult}voltios × PF)\n     = {watts} ÷ ({mult}{volts} × {pf})\n     = {amps} A',
  ampsOhmsFormula: 'Amperios = voltios ÷ {quantity}\n     = {volts} ÷ {ohms}\n     = {amps} A',
  wattsFormula: 'Vatios = {mult}voltios × amperios\n      = {mult}{volts} × {amps}\n      = {watts} W',
  wattsPfFormula: 'Vatios = {mult}voltios × amperios × PF\n      = {mult}{volts} × {amps} × {pf}\n      = {watts} W',
  voltsFormula: 'Voltios = vatios ÷ ({mult}amperios)\n      = {watts} ÷ ({mult}{amps})\n      = {volts} V',
  voltsPfFormula: 'Voltios = vatios ÷ ({mult}amperios × PF)\n      = {watts} ÷ ({mult}{amps} × {pf})\n      = {volts} V',
  voltsOhmsFormula: 'Voltios = amperios × {quantity}\n        = {amps} × {ohms}\n        = {volts} V',
  resistanceFormula: 'Resistencia = voltios ÷ amperios\n            = {volts} ÷ {amps}\n            = {ohms} Ω',
  impedanceFormula: 'Impedancia = voltios ÷ amperios\n           = {volts} ÷ {amps}\n           = {ohms} Ω\nParte resistiva ≈ impedancia × PF\n                 = {ohms} × {pf}\n                 ≈ {resistance} Ω',
  math: '\n<p>{system}:</p>\n<div class="formula">{formula}</div>\n',
  mathWithPf: '\n<p>{system} con factor de potencia {pf}:</p>\n<div class="formula">{formula}</div>\n<p>Potencia aparente: {mult}{volts} V × {amps} A = {va} VA = <strong>{kva} kVA</strong>.</p>',
};
const POWER_UI_TEXT = {
  current: 'corriente',
  realPower: 'potencia real',
  voltage: 'tensión',
  ...
  amps: 'amperios',
  watts: 'vatios',
  resistanceR: 'Resistencia (R)',
  impedanceZ: 'Impedancia (Z)',
  resistivePartResistance: 'Parte resistiva — Resistencia (R ≈ Z × factor de potencia)',
  apparentPower: 'aparente potencia',
  powerFactor: 'factor de potencia',
  checkInput: 'REVISE LOS DATOS',
  enterPositiveValues: 'Ingrese un valor mayor que cero en cada campo visible.',
  currentMustBeGreaterThanZero: 'La corriente debe ser mayor que cero para calcular la resistencia o la impedancia. Ingrese los amperios medidos.',
  ohmsMustBeGreaterThanZero: 'Ingrese un valor superior a 0 Ω. 0 Ω es un cortocircuito y haría infinita la corriente calculada.',
  nextStep: 'Siguiente paso: compare esta corriente con el calibre y la ampacidad antes de elegir un conductor.',
};
...
function applyMode() {
  ...
  $('pw-ohms-label').textContent = system === 'dc' ? 'Resistencia (Ω)' : 'Impedancia (Ω)';
