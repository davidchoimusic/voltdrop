...
function calc() {
  ...
  $('big-label').textContent = vdFormat(POWER_RESULT_TEXT.resultLabel, {
    result: { amps: 'corriente', watts: 'potencia real', volts: 'tensión' }[find],
    system: sys.name,
  });

  const cells = [
    ['voltios', vdFormat(POWER_RESULT_TEXT.volts, { volts: fmt(volts, 1) })],
    ['amperios', vdFormat(POWER_RESULT_TEXT.amps, { amps: fmt(amps) })],
    ['vatios', vdFormat(POWER_RESULT_TEXT.wattsAndKilowatts, {
      watts: fmt(watts, 1),
      kilowatts: fmt(kw, 3),
    })],
  ];
  if (sys.usesPF) {
    cells.push(['aparente potencia', vdFormat(POWER_RESULT_TEXT.kilovoltAmps, { kva: fmt(kva, 3) })]);
    cells.push(['factor de potencia', fmt(pf, 2)]);
  }
  ...
  $('verdict-note').textContent = find === 'amps'
    ? 'Siguiente paso: compare esta corriente con el calibre y la ampacidad antes de elegir un conductor.'
    : '';
