...
const DROP_TEXT = {
  ...
  continuousRuleNote: '<strong>CEC Rule 8-104 额定设备情形——连续负载：导体限于其允许载流量的 80%。</strong>调整前：允许载流量为 {permitted} A。调整后：{permitted} A × {factor}% = {after} A，可供输入的 {load} A 连续负载使用。此开关将全部输入负载视为连续负载；不计算混合负载。',
  continuousRuleMath: '<h3>连续负载步骤——CEC Rule 8-104 额定设备情形</h3>\n<div class="formula">允许载流量：{permitted} A\n× {factor}%\n= {after} A 可用于连续负载\n输入负载 {load} A ≤ {after} A → 通过</div>\n<p>导体限于其允许载流量的 80%。此开关将全部输入负载视为连续负载；不计算混合负载。</p>',
...
function verdictFor(verdict) {
  if (verdict === 'good') return { cls: 'good', badge: '良好', note: '在3%的强制限值内。此线路应能正常工作。' };
  if (verdict === 'warn') return { cls: 'warn', badge: '注意', note: '超出分支电路或馈线3%的强制限值，但仍在组合路径5%的最大值内。请增大导线规格（线规）或缩短线路。' };
  return { cls: 'bad', badge: '过于很多', note: '超出组合路径5%的强制最大值。请增大导线规格（线规）、缩短线路或提高电压。' };
}
