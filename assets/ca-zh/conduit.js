...
const CONDUIT_RESULT_TEXT = {
  ...
  tooLargeMixed: '<p>导线总面积为{area} sq in，超过了4" {conduit}在{percent}%填充限值下可容纳的面积。</p>',
  smallestMixed: '容纳{count}根导线的最小{conduit}',
  ...
  totalBreakdown: '<span><strong>导线总面积</strong></span><span><strong>{area} sq in</strong></span>',
...
const CEC_CONDUIT_RESULT_TEXT = {
  tooLarge: '超过所列最大{conduit}可合规容纳的数量',
  tooLargeMixed: '<p>导线总面积为{area} mm²，超过了{largestSize} {conduit}在{percent}%填充限值下可容纳的面积。</p>',
  smallestMixed: '容纳{count}根导线的最小{conduit}',
  fillAtSize: '{size}的填充率为{percent}%',
  oneWireLimit: '1根导线的限值为{percent}%',
  twoWireLimit: '2根导线的限值为{percent}%',
  ...
  totalBreakdown: '<span><strong>导线总面积</strong></span><span><strong>{area} mm²</strong></span>',
  mathMixed: '\n<p>每一行均采用CSA C22.1:24 {conductorTable}中的导线横截面积。</p>\n<div class="formula">{breakdown}\n总计 = {needed} mm²\n{conduitSize} {conduit}内部面积 = {conduitArea} mm²（根据CSA C22.1:24 Tables 9A–9H中的内径计算）\n按照Rule 12-910和Table 8，{count}根导线允许面积：{conduitArea} × {percent}% = {allowed} mm²\n{needed} ≤ {allowed}  →  实际填充率{actualPercent}%</div>',
};

const CEC_UI_TEXT = {
  conductorTypeLabel: '导线类型',
  conductorTypeHint: 'RW90是加拿大常用的建筑导线。尼龙护套导线请选择T90。',
  rigidPvc: '硬质PVC',
...
function calc() {
  ...
  $('result-grid').innerHTML = [
    ['填充在此规格', vdFormat(resultText.fillAtSize, { percent: pct.toFixed(1), size: pick.size })],
