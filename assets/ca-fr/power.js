...
function calc() {
  ...
  $('big-label').textContent = vdFormat(POWER_RESULT_TEXT.resultLabel, {
    result: { amps: 'courant', watts: 'puissance réelle', volts: 'tension' }[find],
    system: sys.name,
  });

  const cells = [
    ['volts', vdFormat(POWER_RESULT_TEXT.volts, { volts: fmt(volts, 1) })],
    ['ampères', vdFormat(POWER_RESULT_TEXT.amps, { amps: fmt(amps) })],
  ...
  if (sys.usesPF) {
    cells.push(['apparente puissance', vdFormat(POWER_RESULT_TEXT.kilovoltAmps, { kva: fmt(kva, 3) })]);
    cells.push(['facteur de puissance', fmt(pf, 2)]);
  }
  ...
  $('verdict-note').textContent = find === 'amps'
    ? 'Étape suivante : comparez ce courant au calibre et au courant admissible avant de choisir un conducteur.'
    : '';
