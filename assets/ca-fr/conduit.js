...
const CEC_CONDUIT_RESULT_TEXT = {
  tooLarge: 'plus que ce que le plus grand {conduit} répertorié peut contenir légalement',
  tooLargeMixed: '<p>L’aire combinée des conducteurs est de {area} mm², ce qui dépasse la limite de remplissage de {percent}% même dans le {conduit} de {largestSize}.</p>',
  smallestMixed: 'plus petit conduit {conduit} pour {count} conducteurs',
  fillAtSize: '{percent}% de {size}',
  oneWireLimit: '{percent}% pour 1 conducteur',
  twoWireLimit: '{percent}% pour 2 conducteurs',
  ...
  totalBreakdown: '<span><strong>AIRE TOTALE DES CONDUCTEURS</strong></span><span><strong>{area} mm²</strong></span>',
  mathMixed: '\n<p>Chaque ligne utilise la section du conducteur de CSA C22.1:24 {conductorTable}.</p>\n<div class="formula">{breakdown}\nTOTAL = {needed} mm²\nAire intérieure du conduit {conduitSize} {conduit} = {conduitArea} mm² (dérivée de son diamètre intérieur dans CSA C22.1:24 Tables 9A–9H)\nPermis par Rule 12-910 et Table 8 pour {count} conducteurs : {conduitArea} × {percent}% = {allowed} mm²\n{needed} ≤ {allowed}  →  remplissage réel de {actualPercent}%</div>',
};

const CEC_UI_TEXT = {
  conductorTypeLabel: 'Type de conducteur',
  conductorTypeHint: 'RW90 est le conducteur de bâtiment courant au Canada. Choisissez T90 pour un conducteur gainé de nylon.',
  rigidPvc: 'PVC rigide',
...
function calc() {
  ...
  $('result-grid').innerHTML = [
    ['remplissage à ce calibre', vdFormat(resultText.fillAtSize, { percent: pct.toFixed(1), size: pick.size })],
    ['Limite permise', vdFormat(limitPattern, { percent: Math.round(limit * 100) })],
    ['aire totale des conducteurs', vdFormat(
      country === 'ca' ? resultText.squareMillimetres : resultText.squareInches,
      { area: needed.toFixed(gridDecimals) },
    )],
    nextUp
      ? ['Tirage plus facile : diamètre suivant', nextUp[0]]
      : ['ce est le plus grand calibre', family.sizes.at(-1)[0]],
